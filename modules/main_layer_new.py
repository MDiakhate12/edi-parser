...
class MainLayer():
    ...
    def __output_CPLEX_input_container_csvs(self, df_containers: pd.DataFrame, df_filled_slots: pd.DataFrame, df_DG_classes_expanded: pd.DataFrame, d_iso_codes_map: dict) :
        # loading stacks
    ...
    def __run_first_execution(self) -> None:
        ...
        ref_df_subbays_capacity = self.__DL.read_csv(subbays_capacity_csv_path, DEFAULT_MISSING, ";", self.__s3_bucket_in).astype(str)
        ...
        l_tanks_baplies_paths = [], [], []



        # print(l_dfs_containers) # LOC_147_FUNCTION_CODE_QUALIFIER LOC_147_ID LOC_147_CODE_LIST_ID_CODE
        # print(l_containers_folder_names) # ['00_FRLEH', '01_DEHAM']
        # print(l_dfs_rotation_containers) #  EQD_ID LOC_9_LOCATION_ID LOC_11_LOCATION_ID
        # print(l_dfs_tanks) # LOC_ZZZ_FUNCTION_CODE_QUALIFIER LOC_ZZZ_ID LOC_ZZZ_CODE_LIST_ID_CODE
        # print(l_tanks_baplies_paths) # ['../preprocessing_v2/data/simulations/simulation_225_prod/in_preprocessing/call_01_DEHAM/Tank.edi']
        # print(l_tanks_folder_names) # ['01_DEHAM']
        ...
        df_restow = restow_layer.get_df_restow(
            df_final_containers=df_containers, 
            df_stacks_input=ref_df_stacks, 
            df_subbays_capacity_input=ref_df_subbays_capacity,
        )
    ...
    def __update_csvs_with_CPLEX_output(self, d_cplex_containers_slot_by_id: dict, d_cplex_containers_slot_by_id_keys: dict) -> 'tuple[pd.DataFrame, pd.DataFrame]':
        ...
        df_containers = pd.concat(l_dfs_containers, axis=0, ignore_index=True)
        df_containers.fillna("", inplace=True)
        self.__DL.write_csv(df_containers, self.__all_containers_csv_path, s3_bucket=self.__s3_bucket_out)
        return df_containers, df_filled_slots
    ...
    def __run_reexecution(self) -> None:
        ...
        df_containers, df_filled_slots = self.__update_csvs_with_CPLEX_output(d_cplex_containers_slot_by_id, d_cplex_containers_slot_by_id_keys)
            # df_DG_classes_expanded = self.__get_df_DG_classes_expanded()
            # # df_DG_classes_grouped = self.__PL.get_df_DG_classes_grouped(df_containers, df_DG_classes_expanded)
            # self.__output_CPLEX_input_container_csvs(df_containers, df_filled_slots, df_DG_classes_expanded, d_iso_codes_map)
